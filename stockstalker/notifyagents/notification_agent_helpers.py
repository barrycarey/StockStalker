...
def notification_agent_factory(configs: List[dict]) -> List[NotificationAgent]:
    ...
    for config in configs:
        ...
        except InvalidNotificationAgentConfig:
            log.error('Invalid notification agent config')

    return agents

def get_discord_agent(config: dict) -> DiscordAgent:
    if 'webhook' not in config or not config['webhook']:
        raise InvalidNotificationAgentConfig('Discord config is missing webhook URL')
